...
def solve_round1_closed_form(sc: Scenario) -> dict:
    ...
    # With no demand and positive salvage, every brewed unit has positive
    # value; otherwise the normal newsvendor optimum is finite.
...
def solve_round_scipy(sc: Scenario, round_number: int) -> dict:
    ...
    # Demand-centered and zero starts are both feasible after clipping/scaling.
    starts = [
...
def solve_gurobi_round(sc: Scenario, round_number: int, pwl_points: int = 20001) -> dict:
    ...
    CDF/PDF used by the closed-form newsvendor expression. Therefore this adapter
    represents the *same analytic objective* with dense PWL epigraph functions.
    The analytic Python implementation remains the reference truth; this adapter
    is a solver validation layer and never claims an exact closed-form Gurobi
    encoding of Phi/Phi'.
    """
    if round_number not in (1, 2, 3, 4):

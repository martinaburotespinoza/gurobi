...
audit = {
    "schema": "gurobean.r5.1.targeted_audit.v1",
    "seed": SEED,
    "cases": CASES,
    "points": POINTS,
    "targets": {},
...
for case in selected:
    ...
    target = {
        ...
        "scenario": dump_scenario(case.scenario),
        "scipy": {
    ...
    for points in POINTS:
        ...
        try:
            ...
            row = {
                ...
                "objective_error": abs(
                    float(sol["objective"]) - float(scipy["objective"])
                ),
                "Q_hot_error": abs(
                    float(sol["Q_hot"]) - float(scipy["Q_hot"])
                ),
                "Q_cold_error": abs(
                    float(sol["Q_cold"]) - float(scipy["Q_cold"])
                ),
            ...
            print(
                f"PWL={points:6d} "
                f"obj={row['objective']:.12f} "
                f"err={row['objective_error']:.9g} "
                f"qh={row['Q_hot']:.9f} "
                f"qc={row['Q_cold']:.9f} "
                f"time={elapsed:.3f}s"
            )
    ...
    # Repeat the most suspicious resolution to detect nondeterminism.
    repeat_points = 40001
    repeats = []
    ...
    if len(valid) >= 2:
        obj_values = [x["objective"] for x in valid]
        qh_values = [x["Q_hot"] for x in valid]
        qc_values = [x["Q_cold"] for x in valid]

        target["repeat_spread"] = {
            "objective": max(obj_values) - min(obj_values),
            "Q_hot": max(qh_values) - min(qh_values),
            "Q_cold": max(qc_values) - min(qc_values),
        }

        print(
            "spread: "
            f"obj={target['repeat_spread']['objective']:.12g} "
            f"qh={target['repeat_spread']['Q_hot']:.12g} "
            f"qc={target['repeat_spread']['Q_cold']:.12g}"
        )

    audit["targets"][case.label] = target


Path("r5_targeted_audit.json").write_text(

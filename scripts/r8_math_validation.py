...
def _check_single_product(lam: float, revenue: float, cost: float, salvage: float, q: float) -> dict:
    ...
    return {
        "gradient_abs_error": abs(analytic_g - numeric_g),
        "hessian_abs_error": abs(analytic_h - numeric_h),
        "hessian_value": analytic_h,
        "concave": analytic_h <= CONCAVITY_TOL,
    }
...
def audit_case(index: int, sc: Scenario) -> CaseResult:
    try:
        points = []
        for lam, rev, cost, salvage in (
            (sc.lambda_hot, sc.revenue_hot, sc.cost_hot, sc.salvage_hot),
        ...
        checks = {
            ...
            "max_hessian_abs_error": hess_err,
            "all_single_product_objectives_concave": concave,
            "closed_form_stationarity": closed,
            "round_objective_decomposition": decomp,
            "resource_boundary": boundary,
        }
        passed = bool(
            grad_err <= GRAD_TOL
            and hess_err <= HESS_TOL
            and concave
            and closed["stationary"]
...
def main() -> int:
    ...
    out = {
        ...
        "max_gradient_abs_error": max_grad,
        "max_hessian_abs_error": max_hess,

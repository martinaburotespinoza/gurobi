...
objective consistency, checks deterministic replay and optionally cross-checks
the real Gurobi/PWL backend. Use ``--require-gurobi`` for the release gate.
"""
...
from gurobean.model import Scenario, _feasible, _round_bounds, _round_objective, solve_round
...
def certify_case(index: int, round_number: int, sc: Scenario, require_gurobi: bool) -> CaseResult:
    try:
        ref = solve_round(round_number, sc, backend="scipy")
        ...
        ref2 = solve_round(round_number, sc, backend="scipy")
        deterministic = (
        ...
        finite = all(math.isfinite(v) for v in (qh, qc, exact_obj))
        reference_ok = bool(finite and feasible and within_box and deterministic and objective_error <= OBJ_TOL)
        ...
        gurobi_ok = bool(
            ...
            and q_error <= Q_TOL and gobj_error <= OBJ_TOL
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    out = {
        "schema": "gurobean.r9.end_to_end.v2",

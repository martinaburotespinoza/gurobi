"""R9 strict end-to-end certification for the R1-R4 Gurobean pipeline.

Authoritative strict gate: independent analytic reference, real Gurobi, native
PWL objective, 20,001 points, regret <= 2e-6 and feasibility <= 1e-8.
"""
...
def _scenario(rng,edge):
 ...
 rh,rc=rng.uniform(1,8),rng.uniform(1,8); ch,cc=rng.uniform(0,.8*rh),rng.uniform(0,.8*rc); bh,bc=rng.uniform(.05,2),rng.uniform(.05,2); wh,wc=rng.uniform(.1,3),rng.uniform(.1,3)
 scale=lam*(.55 if edge==1 else rng.uniform(.8,2)) if edge!=2 else .002
...
def _obj(sc,r,qh,qc): return _round_objective(sc,r in(2,4),r in(3,4),qh,qc)
def _stationary(sc,r,hot):
 ...
 if lam<=0:return 0.
 if expected_newsvendor_gradient(0.,lam,rev,cost,sal)<=0:return 0.
 hi=max(1.,lam)
...
def _ternary(sc,r,a,b):
 ...
 for _ in range(100):
  ...
  if _obj(sc,r,* (a+m1*(b-a))) < _obj(sc,r,* (a+m2*(b-a))):lo=m1
  else:hi=m2
 cand=(lo,(lo+hi)/2,hi,0.,1.); t=max(cand,key=lambda t:_obj(sc,r,*(a+t*(b-a)))); x=a+t*(b-a); return float(_obj(sc,r,*x)),x
def _vertices(sc,r):
...
def _reference(sc,r):
 if r in(1,3):
  hh,_=_round_bounds(sc,False,r in(3,4));q=max(0.,min(hh,_stationary(sc,r,True)));return {'Q_hot':float(q),'Q_cold':0.,'objective':float(_obj(sc,r,q,0.))}
 vs=_vertices(sc,r);hh,ch=_round_bounds(sc,True,r==4); s=np.array([min(hh,_stationary(sc,r,True)),min(ch,_stationary(sc,r,False))]); cand=[]
 if _feasible(*s,sc):cand.append((_obj(sc,r,*s),s))
 cand += [(_obj(sc,r,*v),v) for v in vs]; center=np.mean(np.stack(vs),axis=0); ordered=sorted(vs,key=lambda x:math.atan2(float(x[1]-center[1]),float(x[0]-center[0])))
...
def certify_case(i,r,sc):
 try:
  ref=_reference(sc,r); qh,qc=float(ref['Q_hot']),float(ref['Q_cold']); exact_ref=_obj(sc,r,qh,qc); referr=abs(ref['objective']-exact_ref); ref2=_reference(sc,r);det=abs(qh-ref2['Q_hot'])<=1e-10 and abs(qc-ref2['Q_cold'])<=1e-10 and abs(ref['objective']-ref2['objective'])<=1e-10; hh,ch=_round_bounds(sc,r in(2,4),r in(3,4)); refok=bool(_feasible(qh,qc,sc) and -FEAS_TOL<=qh<=hh+FEAS_TOL and -FEAS_TOL<=qc<=ch+FEAS_TOL and referr<=REFERENCE_OBJ_TOL and det)
  g=solve_gurobi_round(sc,r,pwl_points=R9_PWL_POINTS);gqh,gqc=float(g['Q_hot']),float(g['Q_cold']);exact=_obj(sc,r,gqh,gqc);reg=abs(exact-exact_ref);qerr=max(abs(gqh-qh),abs(gqc-qc)); feas=_strict_feasible(gqh,gqc,sc); gok=bool(feas and math.isfinite(exact) and reg<=STRICT_REGRET_TOL)
...
def main(argv=None):
 ...
 try: import gurobipy; gv=tuple(gurobipy.gurobi.version())
 except Exception as exc:
  if args.require_gurobi: print(f'Gurobi unavailable: {exc}');return 2
  gv=None
 rng=random.Random(SEED);results=[]
 for i in range(CASES):
  sc=_scenario(rng,i%4)
  for r in ROUNDS:results.append(certify_case(i,r,sc))
 rf=[x for x in results if not x.reference_ok];gc=[x for x in results if x.gurobi_checked];gf=[x for x in gc if not x.gurobi_ok];worst=max((x.regret for x in gc),default=0.);status='PASS' if len(results)==400 and not rf and len(gc)==400 and not gf else 'FAIL'
 art={'schema':'gurobean.r9.strict.v3','seed':SEED,'cases':CASES,'rounds':list(ROUNDS),'total_cases':len(results),'gurobi_version':gv,'reference_failures':len(rf),'gurobi_cases_checked':len(gc),'gurobi_failures':len(gf),'worst_regret':worst,'criteria':{'reference_objective_tol':REFERENCE_OBJ_TOL,'strict_regret_tol':STRICT_REGRET_TOL,'feasibility_tol':FEAS_TOL,'pwl_points':R9_PWL_POINTS},'status':status,'failures':[asdict(x) for x in results if not x.reference_ok or not x.gurobi_ok],'results':[asdict(x) for x in results]};Path('r9_end_to_end.json').write_text(json.dumps(art,indent=2),encoding='utf-8')
 print('=== GUROBEAN R9 STRICT END-TO-END ===');print(f'CASES: {CASES} x ROUNDS: {len(ROUNDS)} = {len(results)}');print(f'GUROBI_VERSION: {gv}');print(f'REFERENCE_FAILURES: {len(rf)}');print(f'GUROBI_CASES_CHECKED: {len(gc)}');print(f'GUROBI_FAILURES: {len(gf)}');print(f'WORST_EXACT_REGRET: {worst:.15g}');print(f'STRICT_REGRET_TOL: {STRICT_REGRET_TOL:.15g}');print(f'FEAS_TOL: {FEAS_TOL:.15g}');print(f'PWL_POINTS: {R9_PWL_POINTS}');print(f'R9 STATUS: {status}');print('ARTIFACT: r9_end_to_end.json');return 0 if status=='PASS' else 1

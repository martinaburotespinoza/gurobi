...
Run after applying the native-PWL-objective patch.  The script checks exact
continuous objective regret, resource feasibility, and marginal-value/KKT
alignment at several mesh sizes.  It never changes certification tolerances.
"""
...
def exact_gradient(sc, round_number, qh, qc):
    ...
    gh = expected_newsvendor_gradient(
        qh, sc.lambda_hot, sc.revenue_hot,
        sc.cost_hot if include_cost else 0.0, sc.salvage_hot,
    )
    gc = expected_newsvendor_gradient(
        qc, sc.lambda_cold, sc.revenue_cold,
...
def main() -> int:
    ...
    for i in range(r9.CASES):
        scenarios[i] = r9._scenario(rng, i % 4)
    ...
    for case, rn in sorted(FAIL_CASES):
        ...
        for mesh in MESHES:
            ...
            feasible = (
                -1e-8 <= qh <= hot_hi + 1e-8 and
                -1e-8 <= qc <= cold_hi + 1e-8 and
                beans <= sc.beans_available + 1e-8 and
                water <= sc.water_available + 1e-8
            )
            print(
                f"  mesh={mesh:5d} q=({qh:.12f},{qc:.12f}) "
                f"regret={regret:.12g} bean_gap={bean_lambda_gap:.12g} "
                f"slack=({sc.beans_available-beans:.12g},{sc.water_available-water:.12g}) "
                f"feasible={feasible}"
            )

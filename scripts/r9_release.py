...
def check(case,round_number,params):
    ...
    try:
        ...
        return {'case':case,'round_number':round_number,'pwl_points':PWL_POINTS,'reference_ok':reference_ok,'gurobi_status':status,'feasible':feasible,'exact_objective':exact,'reference_objective':exact_ref,'exact_regret':regret,'solver_objective':solver_obj,'solver_objective_error':solver_obj_error,'q_hot':q,'q_cold':solved.get('q_cold'),'reference_q_hot':rq,'reference_q_cold':reference.get('q_cold'),'q_error':abs(q-rq),'passed':passed,'error':None if passed else 'strict gate failure'}
    except Exception as exc:
        return {'case':case,'round_number':round_number,'pwl_points':PWL_POINTS,'reference_ok':reference_ok,'gurobi_status':None,'feasible':False,'exact_objective':None,'reference_objective':exact_ref,'exact_regret':None,'solver_objective':None,'solver_objective_error':None,'q_hot':None,'q_cold':None,'reference_q_hot':rq,'reference_q_cold':reference.get('q_cold'),'q_error':None,'passed':False,'error':repr(exc)}
...
def main():
    ...
    artifact={'schema':'gurobean.r9.release-certification.v4','git_commit':commit,'cases':r9.CASES,'rounds':list(r9.ROUNDS),'cases_checked':len(records),'pwl_points':PWL_POINTS,'refine_points':list(REFINE_POINTS),'reference_objective_tolerance':REF_TOL,'exact_objective_regret_tolerance':OBJ_TOL,'solver_feasibility_tolerance':1e-8,'gurobi_gate':'CHECKED','r5_r8_status':'CALIBRATION_GATED','failures':len(failures),'max_exact_regret':max(regrets,default=None),'max_solver_objective_error':max(solver_errors,default=None),'records':records,'status':'PASS' if not failures else 'FAIL'}

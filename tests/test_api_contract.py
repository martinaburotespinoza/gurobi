...
from api.app import SolveRequest, app
...
def test_flat_payload_uses_r2_7525_default_when_distribution_is_omitted():
    ...
    normalized = SolveRequest.model_validate(payload)
    assert normalized.scenario.p_hot == 0.75
    assert normalized.scenario.p_cold == 0.25

...
def test_strict_gate_requires_explicit_oos_threshold():
    decision = strict_calibration_gate(_real_obs(), [_fit()], min_observations=8, require_monotone=True)
    assert not decision.eligible
    assert decision.reason == "missing_oos_rmse_threshold"


def test_strict_gate_accepts_real_game_provenance_when_statistics_pass():
    decision = strict_calibration_gate(_real_obs(), [_fit()], min_observations=8, max_rmse=0.1, require_monotone=True)
    assert decision.eligible
    assert decision.reason == "promotion_gate_passed"
    assert decision.metrics["out_of_sample_rmse"] <= 0.1

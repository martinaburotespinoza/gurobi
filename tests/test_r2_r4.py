...
def test_round2_has_two_products_and_shared_resources():
    sc = Scenario(lambda_total=50, p_hot=0.6, p_cold=0.4, revenue_hot=2.0, revenue_cold=2.5, beans_available=60, water_available=100, beans_hot=1, beans_cold=2, water_hot=1, water_cold=1)
...
def test_round3_with_positive_brew_cost_stops_before_infinite_inventory():
    sc = Scenario(lambda_total=40, p_hot=1, revenue_hot=5, cost_hot=2, beans_available=float("inf"), water_available=float("inf"))
...
def test_round4_objective_is_sum_of_product_objectives():
    sc = Scenario(lambda_total=80, p_hot=0.5, p_cold=0.5, revenue_hot=4, revenue_cold=5, cost_hot=1.5, cost_cold=2, beans_available=100, water_available=100, beans_hot=1, beans_cold=1, water_hot=1, water_cold=1)
...
def test_gradient_matches_finite_difference():
    ...
    eps = 1e-5
    fd = (expected_newsvendor_profit(q + eps, lam, rev, cost) - expected_newsvendor_profit(q - eps, lam, rev, cost)) / (2 * eps)

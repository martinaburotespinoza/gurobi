...
def test_simulation_is_bitwise_reproducible_for_same_seed():
    cfg = GurobeanSimulationConfig(
        ...
        p_cold=0.35,
        mu_rate=28.0,
        brew_hot_per_hour=15.0,
        brew_cold_per_hour=10.0,
        revenue_hot=4.0,
        revenue_cold=4.5,
        brew_cost_hot=0.7,
        brew_cost_cold=0.9,
    ...
    first = simulate_gurobean(cfg)
    second = simulate_gurobean(cfg)
    assert first == second
...
def test_simulation_metrics_obey_basic_bounds():
    cfg = GurobeanSimulationConfig(
        ...
        p_cold=0.3,
        mu_rate=45.0,
        brew_hot_per_hour=40.0,
        brew_cold_per_hour=20.0,
        revenue_hot=5.0,
        revenue_cold=5.5,
        brew_cost_hot=1.0,
        brew_cost_cold=1.2,
...
def test_multicup_sampler_never_creates_zero_or_negative_orders():
    ...
    cfg = GurobeanSimulationConfig(
        ...
        p_hot=1.0,
        p_cold=0.0,
        order_size_sampler=bad_sampler,
    )
